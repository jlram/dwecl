...
(function () {
    ...
    texto.textContent += ' ' + number1 + ' + ' + number2;
    ...
    document.getElementById('condiciones').addEventListener('change', function(event){
        ...
        boton.disabled = (event.target.checked ? false : true);
    });
    ...
    function checkForm(event) {
        ...
        var isValidName = checkName(nombre, 3, 15);
        var isValidPass = checkPassword(passwd);
        var isValidEmail = checkMail(document.getElementById('email'));
        if (isValidPass){
    ...
    let number1 = generateNumber(0, 10);
    let number2 = generateNumber(0, 10);
